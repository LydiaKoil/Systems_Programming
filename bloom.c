...
{
	struct bloom *myBloom=(struct bloom *)malloc(sizeof(struct bloom));
	myBloom->size=size;
	myBloom->bit_vector=bitset_new(myBloom->size); 
	return myBloom;
...
int bloom_lookup(struct bloom * this, char * item)
{
	...
	int hash_val_2=hash_string(item,BLOOM_HASH2,this->size);

	if(bitset_lookup(this->bit_vector,hash_val_1)==1 && bitset_lookup(this->bit_vector,hash_val_2)==1){
...
{	
	...
	int hash_val_2=hash_string(item,BLOOM_HASH2,this->size);
	//this->bit_vector->
	bitset_add(this->bit_vector,hash_val_1);
	//this->bit_vector->
	bitset_add(this->bit_vector,hash_val_2);
